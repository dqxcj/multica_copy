...
SELECT id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, visibility, label FROM agent_runtime
...
WHERE id = $1
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, visibility, label
`
...
const updateAgentRuntimeLabel = `-- name: UpdateAgentRuntimeLabel :one
UPDATE agent_runtime
...
type UpdateAgentRuntimeLabelParams struct {
	Label string      `json:"label"`
	ID    pgtype.UUID `json:"id"`
}

func (q *Queries) UpdateAgentRuntimeLabel(ctx context.Context, arg UpdateAgentRuntimeLabelParams) (AgentRuntime, error) {
	row := q.db.QueryRow(ctx, updateAgentRuntimeLabel, arg.Label, arg.ID)
	var i AgentRuntime
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
DO UPDATE SET
    ...
    last_seen_at = now(),
    updated_at = now()
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, visibility, label, (xmax = 0) AS inserted
`
